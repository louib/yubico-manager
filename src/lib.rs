...
impl Yubico {
    ...
    pub fn find_yubikey(&mut self) -> Result<Device> {
        for device in self.context.devices().unwrap().iter() {
            ...
            let serial_number =
                device.config_descriptor(descr.serial_number_string_index().unwrap());
    ...
    pub fn find_yubikeys(&mut self) -> Result<Vec<Device>> {
        ...
        for device in self.context.devices().unwrap().iter() {
            let descr = device.device_descriptor().unwrap();
            if descr.vendor_id() == VENDOR_ID {
                let serial_number_string_index = descr.serial_number_string_index().unwrap();
                println!("string index {:?}", serial_number_string_index);
                let serial_number = device.config_descriptor(serial_number_string_index);
